...
router.post('/client', adminKeyMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        next(error);import { Router, Request, Response, NextFunction } from 'express';
import bcrypt from 'bcryptjs';
import { query } from '../db';
...
router.get('/users', adminKeyMiddleware, async (req, res, next) => {
    try {
        const result = await query('SELECT id, email, role, created_at FROM users ORDER BY id');
        res.json(result.rows);
...
router.post('/users', adminKeyMiddleware, async (req, res, next) => {
    const { email, password } = req.body;
...
router.delete('/users/:id', adminKeyMiddleware, async (req, res, next) => {
    const userId = parseInt(req.params.id as string, 10);
...
router.get('/keys', adminKeyMiddleware, async (req, res, next) => {
    try {
        const result = await query(`
            SELECT k.id, k.key_code, k.created_at, u.email AS used_by_email,
                   creator.email AS created_by_email
...
router.post('/keys', adminKeyMiddleware, async (req, res, next) => {
    const { count = 1 } = req.body;
    try {
        const keys = [];
        for (let i = 0; i < count; i++) {
...
router.delete('/keys/:id', adminKeyMiddleware, async (req, res, next) => {
    const keyId = parseInt(req.params.id as string, 10);
...
// ════════════════════════════════════════════════════════════
//  NEW: Add Client (email + password + MT5 credentials)
// ════════════════════════════════════════════════════════════
router.post('/client', adminKeyMiddleware, async (req, res, next) => {
    const { email, password, mt5 } = req.body;
    if (!email || !password || !mt5 || !mt5.login || !mt5.password || !mt5.server) {
        return res.status(400).json({ error: 'Missing required fields' });
    }
    try {
        ...
        // 3. Generate access key and bind to user
        const keyCode = `KEY-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
        await query(
            'INSERT INTO access_keys (key_code, used_by, used_at, created_by) VALUES ($1, $2, NOW(), $3)',
            [keyCode, userId, userId]
